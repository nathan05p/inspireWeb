...
export default function CampNavbar() {
  ...
  return (
    <>
      <motion.header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-[#1A1E22]/90 backdrop-blur-md shadow-sm' : 'bg-transparent'}`}
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            className="fixed inset-0 z-[100] bg-[#1A1E22] flex flex-col"
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
          ...
          >
            {/* Menu header */}
            <div className="flex justify-between items-center px-5 sm:px-8 md:px-16 py-5 border-b border-stone-800 shrink-0">
              <span className="text-stone-400 text-xs tracking-widest">NAVIGATION</span>
              <button
                onClick={() => setMenuOpen(false)}
                className="text-stone-50 hover:text-amber-500 transition-colors p-1"
            ...
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-0 overflow-y-auto">
              {Object.entries(menuSections).map(([section, links], si) => (
                <motion.div
                  key={section}
                  className="border-b sm:border-b-0 sm:border-r border-stone-800 last:border-0 px-5 sm:px-8 md:px-12 py-8 md:py-12"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
            ...
            {/* Menu footer */}
            <div className="px-5 sm:px-8 md:px-16 py-5 border-t border-stone-800 flex items-center justify-center shrink-0">
              <p className="text-stone-400 text-xs tracking-widest text-center">INSPIRE+ TIMIȘOARA</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
