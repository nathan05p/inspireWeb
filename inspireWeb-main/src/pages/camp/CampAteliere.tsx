...
export default function CampAteliere() {
  ...
  return (
    <section id="ateliere" className="py-16 sm:py-24 md:py-32 px-4 sm:px-6 md:px-12 max-w-screen-2xl mx-auto border-b border-[#262E1A]/50 bg-[#0F120A]">
        ...
        <div className="lg:col-span-4 relative">
          <div className="sticky top-32">
            <motion.h2 
              ...
              viewport={{ once: true }}
              className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-outfit font-bold tracking-tight text-[#E8ECD7] leading-none"
            >
              Ateliere
            </motion.h2>
            <motion.div
            ...
            >
              <p className="text-mocha-accent text-sm font-bold tracking-[0.2em] uppercase">
                inspire+ 2026 — The Cross
              </p>
              <p className="text-[#9CA691] text-base sm:text-lg leading-relaxed">
              ...
              <p className="text-[#9CA691] text-sm sm:text-base leading-relaxed">
              ...
              <p className="text-[#E8ECD7]0 text-sm italic mt-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="hidden sm:flex items-start shrink-0 w-16 pt-6 relative z-10">
                    <div className={`w-4 h-4 rounded-full border-2 transition-colors duration-500 relative bg-[#0F120A] mx-auto ${isExpanded ? 'border-mocha-accent shadow-[0_0_10px_rgba(245,158,11,0.5)]' : 'border-[#3D4A29] group-hover:border-[#556639]'}`} />
                  ...
                  <motion.div 
                    ...
                    className={`flex-1 overflow-hidden rounded-3xl sm:rounded-[2rem] border transition-all duration-500 ${isExpanded ? 'bg-[#181D10] border-mocha-accent/40 shadow-2xl' : 'bg-transparent border-transparent cursor-pointer hover:bg-[#181D10]/40'}`}
                  >
                    ...
                    <motion.div layout className={`p-4 sm:p-6 lg:p-8 flex items-center justify-between gap-4 ${isExpanded ? 'pb-4 sm:pb-6' : ''}`}>
                      <motion.h3 
                        layout="position"
                        className={`font-outfit font-bold tracking-tight lowercase transition-colors duration-500 break-words flex-1 ${isExpanded ? 'text-3xl sm:text-4xl md:text-5xl text-mocha-accent' : 'text-3xl sm:text-5xl lg:text-6xl text-[#BCC5B1] group-hover:text-[#E8ECD7]'}`}
                      >
                        {atelier.title}
                      </motion.h3>
                      ...
                      {!isExpanded && (
                        <motion.div
                          ...
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
                          className="shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full border border-[#262E1A]/50 flex items-center justify-center text-[#E8ECD7]0 group-hover:text-mocha-accent group-hover:border-mocha-accent/50 transition-all shadow-sm"
                      ...
                      {isExpanded && (
                        <motion.button
                          ...
                          className="shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-[#0F120A] border border-[#262E1A]/80 flex items-center justify-center text-[#9CA691] hover:text-[#E8ECD7] hover:bg-[#262E1A] hover:border-[#3D4A29] transition-all shadow-sm"
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        ...
                        >
                          ...
                          >
                            <p className="text-[#BCC5B1] text-sm sm:text-base md:text-lg leading-relaxed md:leading-loose whitespace-pre-wrap">
                              {atelier.desc}
                            </p>
                          </motion.div>
                        </motion.div>
